using System;
using System.Collections.Generic;

namespace CMG.DataAccess.Domain
{
    public partial class Cases
    {
        public Cases()
        {
            CaseAgents = new HashSet<CaseAgent>();
        }

        public int Keycase { get; set; }
        public int Keynump { get; set; }
        public decimal CaseStage { get; set; }
        public int Case1 { get; set; }
        public int Case2 { get; set; }
        public int Case4 { get; set; }
        public int Case3 { get; set; }
        public int Case5 { get; set; }
        public int Case6 { get; set; }
        public int Case7 { get; set; }
        public int Case8 { get; set; }
        public DateTime? Case1d { get; set; }
        public DateTime? Case2d { get; set; }
        public DateTime? Case3d { get; set; }
        public DateTime? Case4d { get; set; }
        public DateTime? Case5d { get; set; }
        public DateTime? Case6d { get; set; }
        public DateTime? Case7d { get; set; }
        public DateTime? Case8d { get; set; }
        public bool Casenew { get; set; }
        public string Casey1 { get; set; }
        public string Casey2 { get; set; }
        public string Casey3 { get; set; }
        public string Casey4 { get; set; }
        public string Casey5 { get; set; }
        public string Casey6 { get; set; }
        public string Casey7 { get; set; }
        public string Casey8 { get; set; }
        public DateTime? Tdate { get; set; }
        public string RevLocn { get; set; }
        public DateTime RevDate { get; set; }
        public string Cr8Locn { get; set; }
        public DateTime Cr8Date { get; set; }
        public bool Del { get; set; }
        public int Num { get; set; }
        public bool Casehilite { get; set; }
        public string Title { get; set; }
        public string SalesforceId { get; set; }
        public int? Dictas { get; set; }

        public virtual People People { get; set; }
        public virtual IEnumerable<CaseAgent> CaseAgents { get; set; }
    }
}
